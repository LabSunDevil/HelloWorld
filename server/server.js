...
app.post('/api/videos/:id/view', (req, res) => {
    ...
    // SENTINEL: Validate input to prevent malicious payloads
...
app.get('/api/recommendations', (req, res) => {
    ...
    db.all(`SELECT videoId FROM views WHERE userId = ? ORDER BY timestamp DESC LIMIT 10`, [userId], (err, viewRows) => {
        ...
        db.get(`SELECT tags FROM videos WHERE id = ?`, [watchedVideoIds[0]], (err, video) => {
             // SENTINEL: Use parameterized queries for dynamic IN clause
             const placeholdersList = watchedVideoIds.map(() => '?').join(',');

             if (err || !video) {
                 // Fallback
                 db.all(`SELECT videos.*, users.username as uploaderName FROM videos LEFT JOIN users ON videos.uploaderId = users.id WHERE videos.id NOT IN (${placeholdersList}) ORDER BY RANDOM() LIMIT 5`, watchedVideoIds, (err, rows) => {
                     if(err) return res.status(500).json({error: err.message});
                     res.json(rows);
                 });
                 return;
             ...
             // exclude watched
             const excludeClause = `AND id NOT IN (${placeholdersList})`;
             const allParams = [...params, ...watchedVideoIds];
             ...
             db.all(sql, allParams, (err, recRows) => {
                 if (err) {
                      // Fallback
                     db.all(`SELECT videos.*, users.username as uploaderName FROM videos LEFT JOIN users ON videos.uploaderId = users.id WHERE videos.id NOT IN (${placeholdersList}) ORDER BY RANDOM() LIMIT 5`, watchedVideoIds, (err, rows) => {
                         if(err) return res.status(500).json({error: err.message});
                 ...
                 if (recRows.length < 5) {
                     // Fill with random videos if recommendations are not enough
                      db.all(`SELECT videos.*, users.username as uploaderName FROM videos LEFT JOIN users ON videos.uploaderId = users.id WHERE videos.id NOT IN (${placeholdersList}) ORDER BY RANDOM() LIMIT ?`, [...watchedVideoIds, 5 - recRows.length], (err, randomRows) => {
                         if(err) return res.status(500).json({error: err.message});
