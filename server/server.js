             ...
             db.all(sql, allParams, (err, recRows) => {
                 ...
                 if (recRows.length < 5) {
                     // Fill with random videos if recommendations are not enough
                      ...
                      db.all(`SELECT videos.*, users.username as uploaderName FROM videos LEFT JOIN users ON videos.uploaderId = users.id WHERE videos.id NOT IN (${randomPlaceholders}) ORDER BY RANDOM() LIMIT ?`, [...watchedVideoIds, limit], (err, randomRows) => {
                         if(err) return res.status(500).json({error: err.message});
