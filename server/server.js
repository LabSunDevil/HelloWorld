...
app.get('/api/recommendations', (req, res) => {
    ...
    db.all(`SELECT videoId FROM views WHERE userId = ? ORDER BY timestamp DESC LIMIT 10`, [userId], (err, viewRows) => {
        ...
        db.get(`SELECT tags FROM videos WHERE id = ?`, [watchedVideoIds[0]], (err, video) => {
             ...
             if (err || !video) {
                 // Fallback
                 db.all(`SELECT videos.*, users.username as uploaderName FROM videos LEFT JOIN users ON videos.uploaderId = users.id WHERE videos.id NOT IN (${watchedPlaceholders}) ORDER BY RANDOM() LIMIT 5`, watchedVideoIds, (err, rows) => {
                     if(err) return res.status(500).json({error: err.message});
                     res.json(rows);
                 });
                 return;
             ...
             const likePlaceholders = tags.map(() => `tags LIKE ?`).join(' OR ');
             let params = tags.map(t => `%${t}%`);
             params = params.concat(watchedVideoIds);

             // exclude watched
             const excludeClause = `AND id NOT IN (${watchedPlaceholders})`;

             const sql = `SELECT videos.*, users.username as uploaderName FROM videos LEFT JOIN users ON videos.uploaderId = users.id WHERE (${likePlaceholders}) ${excludeClause} LIMIT 5`;
             ...
             db.all(sql, params, (err, recRows) => {
                 ...
                 if (recRows.length < 5) {
                     // Fill with random videos if recommendations are not enough
                      const fallbackParams = [...watchedVideoIds, 5 - recRows.length];
                      db.all(`SELECT videos.*, users.username as uploaderName FROM videos LEFT JOIN users ON videos.uploaderId = users.id WHERE videos.id NOT IN (${watchedPlaceholders}) ORDER BY RANDOM() LIMIT ?`, fallbackParams, (err, randomRows) => {
                         if(err) return res.status(500).json({error: err.message});
