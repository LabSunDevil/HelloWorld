             ...
             db.all(sql, params, (err, recRows) => {
                 ...
                 if (recRows.length < 5) {
                     // Fill with random videos if recommendations are not enough
                      ...
                      db.all(`SELECT videos.*, users.username as uploaderName FROM videos LEFT JOIN users ON videos.uploaderId = users.id WHERE videos.id NOT IN (${watchedPlaceholders}) ORDER BY RANDOM() LIMIT ?`, randomParams, (err, randomRows) => {
                         if(err) return res.status(500).json({error: err.message});
