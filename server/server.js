...
app.get('/api/recommendations', (req, res) => {
    ...
    db.all(`SELECT videoId FROM views WHERE userId = ? ORDER BY timestamp DESC LIMIT 10`, [userId], (err, viewRows) => {
        ...
        db.get(`SELECT tags FROM videos WHERE id = ?`, [watchedVideoIds[0]], (err, video) => {
             ...
             const excludeClause = `AND videos.id NOT IN (${excludePlaceholders})`;

             const sql = `SELECT videos.*, users.username as uploaderName FROM videos LEFT JOIN users ON videos.uploaderId = users.id WHERE (${tagConditions}) ${excludeClause} LIMIT 5`;
             const allParams = [...tagParams, ...watchedVideoIds];
             ...
             db.all(sql, allParams, (err, recRows) => {
                 ...
                 if (recRows.length < 5) {
                     // Fill with random videos if recommendations are not enough
                      const placeholders = watchedVideoIds.map(() => '?').join(',');
                      db.all(`SELECT videos.*, users.username as uploaderName FROM videos LEFT JOIN users ON videos.uploaderId = users.id WHERE videos.id NOT IN (${placeholders}) ORDER BY RANDOM() LIMIT ?`, [...watchedVideoIds, 5 - recRows.length], (err, randomRows) => {
                         if(err) return res.status(500).json({error: err.message});
